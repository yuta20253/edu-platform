...
export async function GET(req: Request, { params }: { params: Promise<{ draftTaskId: string }> }){
    try {
        const { draftTaskId } = await params

        const { status, data, setCookie } = await railsFetch(`/api/v1/student/draft_tasks/${draftTaskId}`, {
            method: "GET"
        })
