...
const kpiCards = (stats: AdminDashboardData['stats']) => [
  { label: '生徒数', value: stats.student_count, icon: <SchoolIcon />, color: '#2563eb' },
  { label: '教師数', value: stats.teacher_count, icon: <PeopleIcon />, color: '#7c3aed' },
  { label: '総問題数', value: stats.total_questions, icon: <QuizIcon />, color: '#d97706' },
  { label: '管理者数', value: stats.admin_count, icon: <AdminPanelSettingsIcon />, color: '#059669' },
]
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 2, mb: 4 }}>
        {kpiCards(stats).map((card) => (
          <Card key={card.label} elevation={0} sx={{ border: '1px solid #e2e8f0', borderRadius: 2 }}>
